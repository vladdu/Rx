...
class SchemaTypeMismatch(SchemaMismatch):
    def __init__(self, name, desired_type):
        SchemaMismatch.__init__(self,
                                '{0} must be {1}'.format(name, desired_type))
    ...
    def make_range_validator(opt):
        ...
        def validate_range(value, name='value'):
            if not check_range(value):
                ...
                raise SchemaRangeMismatch(name + ' must be in range ' +
                                          range_str)
...
class Factory(object):
    ...
    def expand_uri(self, type_name):
        ...
        if prefix not in self.prefix_registry:
            raise KeyError("unknown prefix '{0}' in type name '{1}'".format(
                prefix, type_name))
    ...
    def learn_type(self, uri, schema):
        if self.type_registry.get(uri):
            raise SchemaError(
                "tried to learn type for already-registered uri {0}".format(
                    uri))
...
class _CoreType(object):
    ...
    def __init__(self, schema, rx):
        if not {'type'}.issuperset(schema):
            raise SchemaError('unknown parameter for //{0}'.format(
                self.subname()))
...
class IntType(_CoreType):
    ...
    def validate(self, value, name='value'):
        if not isinstance(value, Number) or isinstance(value,
                                                       bool) or value % 1:
...
core_types = [
    AllType, AnyType, ArrType, BoolType, DefType, FailType, IntType, MapType,
    NilType, NumType, OneType, RecType, SeqType, StrType
]
